...
const setPlugin = function setPlugin(plugin) {
  globalPlugin = plugin;
  const orgArrLen = instanceArr.length;
  for (let i = 0; i < orgArrLen; i++) {
    const orgInstance = instanceArr[i];
    instance(orgInstance.file, plugin, orgInstance);
  }
  instanceArr.splice(0, orgArrLen);
  return this;
};
 ...
 * @param {object} plugin 插件,可选
 * @param {object} orgInstance 原instance，用于instance更新
 * @param {Function} plugin 处理插件,当没有时使用console方式处理
 * @return {Object}
 */
const instance = function(fileName, plugin = globalPlugin, orgInstance = null) {
  const me = orgInstance || {};
